/**
 * Export Large 60″ diagram callout circles (white rim + drop shadow) as PNG.
 * Run: node scripts/export-large-60-callouts.mjs
 */
...
const OUT_DIR = path.join(ROOT, "exports", "large-60-callouts");
const PRODUCT_DIR = path.join(ROOT, "public", "products", "lay-n-go-large-pdp");

/** Matches large diagram stage (`bg-white`). */
const PAGE_BG = "#ffffff";

/** Matches LayNGoLargeCalloutDiagram large thumbs at md (`h-40 w-40`, default object-cover). */
const CALLOUTS = [
  {
    key: "cord-pocket",
...
async function main() {
  ...
  for (const callout of CALLOUTS) {
    ...
    const base = `large-60-${callout.key}-circle`;

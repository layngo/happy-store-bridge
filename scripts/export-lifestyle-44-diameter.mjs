/**
 * Export Lay-n-Go Lifestyle 44" diameter measure graphic as PNG.
 * Run: node scripts/export-lifestyle-44-diameter.mjs
 */
...
const OUT_DIR = path.join(ROOT, "exports", "lifestyle-44-diameter");
const HERO = path.join(ROOT, "public", "products", "lay-n-go-lifestyle-44", "hero-callout-main.png");

/** Matches lifestyle diagram stage width (`w-[min(64vw,624px)]`). */
const MEASURE_WIDTH_PX = 624;
/** Matches `DiameterLine` lifestyle bracket at md (`w-[min(100%,86%)]`). */
const BRACKET_WIDTH_PCT = 86;
/** Matches `h-9` tick height at md. */
const TICK_HEIGHT_PX = 36;
...
async function main() {
  ...
  exported.push(
    await exportPng(page, "lifestyle-44-diameter-measure", { heroSrc, withHero: false }),
  );
  exported.push(
    await exportPng(page, "lifestyle-44-hero-with-diameter", { heroSrc, withHero: true }),
  );

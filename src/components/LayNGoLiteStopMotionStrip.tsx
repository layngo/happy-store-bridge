...
function StopMotionPlayer({ frames, frameIndex, className }: StopMotionPlayerProps) {
  return (
    <div
      className={cn(
        "relative flex aspect-square w-full max-w-[min(100%,20rem)] items-center justify-center sm:max-w-xs md:max-w-sm",
        className,
      )}
    >
      {frames.map((frame, i) => (
  ...
  return (
    ...
    >
      <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:max-w-7xl">
        <div className="flex flex-col items-stretch gap-8 sm:gap-10 md:flex-row md:items-center md:justify-center md:gap-6 lg:gap-12">
          <StopMotionPlayer frames={COLOR_A_FRAMES} frameIndex={frameIndex} className="mx-auto md:mx-0" />
          <StopMotionPlayer frames={COLOR_B_FRAMES} frameIndex={frameIndex} className="mx-auto md:mx-0" />
        </div>

        <div className="mt-8 text-center sm:mt-10">
          <p className="brand-eyebrow text-foreground">Reversible colors</p>
          <p className="brand-display mx-auto mt-2 max-w-xl text-[clamp(1.25rem,4.5vw,2rem)] leading-tight text-foreground">

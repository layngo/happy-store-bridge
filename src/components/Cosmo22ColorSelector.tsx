...
export function Cosmo22ColorSelector({ product, selectedVariantIdx, onVariantChange }: Cosmo22ColorSelectorProps) {
  ...
  const selectedColor =
    selectedVariant?.selectedOptions.find((o) => /color|colour/i.test(o.name))?.value ?? COSMO_22_SWATCHES[0].shopifyColor;
...
function getSwatchSurfaceStyle(def: Cosmo22SwatchDef): CSSProperties {
  return {
...
export function getCosmo22HeroImageUrls(shopifyColor: string): string[] {
